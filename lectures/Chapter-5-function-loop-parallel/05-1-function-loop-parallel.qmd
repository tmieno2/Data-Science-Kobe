...
microbenchmark(
  ...
  #--- not vectorized ---#
  "not vectorized" = {
    rev_data <- bind_rows(lapply(seq_len(nrow(parameters_data)), gen_rev_corn))
  },
  times = 100,
  unit = "ms"
)
...
#| cache: true
tic()
MC_results <- future_lapply(1:1000, MC_sim, future.seed = TRUE)
toc()
```
...
3. Find the optimal N rate for each combination of corn price and nitrogen price
...
+ Vectorized version is more memory-hungry:
  * Strategy 1: avoids the full nitrogen-by-price grid, but peak memory includes concurrent worker grids and all returned optimum rows
