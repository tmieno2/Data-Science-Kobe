...
<!-- CHANGED (A2): the original pasted
     "~/Dropbox/TeachingUNL/Data-Science-with-R-Quarto/Lectures/..." into chunks
     that actually ran, so the lecture only rendered on Taro's computer. A
     revision then replaced that with a hidden dir_ch3 <- here::here(...) chunk,
     which was just as unusable for a student: every example on the slide read
     through a variable they had never seen defined. Every example now reads the
     plain file name, exactly as a student runs it with the data file in their
     working directory. That is also what Quarto uses when it renders, because
     the working directory is the folder holding this .qmd, and the data files
     sit next to it. -->
...
```{r}
#| label: read-excel-by-name
corn_08 <- read_excel("corn_yields.xls", sheet = "corn_yields_08")
```
<!-- CHANGED (A12): this said sheet = "2008", which is not a sheet in the
     workbook; the sheets are corn_yields_08 and corn_yields_09, as the
     excel_sheets() output above now shows. It never errored only because the
     chunk did not run. -->
