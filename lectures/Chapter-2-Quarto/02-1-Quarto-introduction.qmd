...
| `[Kobe University](https://www.kobe-u.ac.jp)` | a web link |
...
+ In its repository, `sample_qmd.qmd` is located in the <span style = "color: blue;"> `templates` </span> folder.
<!-- CHANGED (A5): was here::here("Chapter-2-Quarto"), which printed a path that
     ...
     real, so the slide was confidently displaying a fake location. The sample
     file is in the templates folder of its companion repository. -->

+ This means that R tries to find <span style = "color: blue;"> `corn_price.csv` in that same `templates` folder</span>.
